...
class FortniteTrackerScraper {
    constructor(options = {}) {
        ...
        try {
            const rosterPath = path.join(__dirname, 'config', 'roster.json');
            const rosterRaw  = require('fs').readFileSync(rosterPath, 'utf8');
            const roster     = JSON.parse(rosterRaw);
            this.players = roster
                .filter(p => p.ftTrackerUrl)
                .map(p => ({
                    name:     p.name,
                    url:      p.ftTrackerUrl,
                    username: p.ftTrackerUsername,
                    platform: p.ftPlatform,
                    region:   p.ftRegion || undefined
                }));
        } catch (e) {
            console.error('Errore lettura roster.json:', e.message);
            this.players = [];
        }
    ...
    initScheduler() {
        console.log('Fortnite Tracker Scraper inizializzato');
        console.log('Player monitorati:', this.players.length);
        console.log('Orario update automatico: 01:00');
    ...
    async runScraping(trigger = 'manual') {
        if (this.isRunning) {
            this.log('Scraping già in corso, saltato');
            return;
        }

        // Ricarica roster.json ad ogni scansione per includere nuovi player
        try {
            const rosterPath = require('path').join(__dirname, 'config', 'roster.json');
            const rosterRaw  = require('fs').readFileSync(rosterPath, 'utf8');
            const roster     = JSON.parse(rosterRaw);
            this.players = roster
                .filter(p => p.ftTrackerUrl)
    ...
    async saveStats(statsArray) {
        try {
            const trimmedStats = statsArray.map(player => ({
                ...player,
                stats: {
                    ...player.stats,
                    tournaments: (player.stats?.tournaments || [])
                        .filter(t => t.placement <= 500)
                        .map(({ name, placement }) => ({ name, placement })) // solo name e placement
                        .slice(0, 10)
                }
            }));
            ...
            await fs.writeFile(this.dataFile, JSON.stringify({
                ...
                players:    trimmedStats
            }, null, 2), 'utf8');

            this.log(`Stats salvate: ${this.dataFile}`);
        } catch (error) {
