...
async function loadSiteLinks() {
    try {
        ...
        // 1. Navigazione Header (Ora statica in HTML come richiesto)
        // highlightActiveNavLink() viene chiamato comunque per gestire lo stato 'active' dei link fissi
        highlightActiveNavLink();
        
        // 2. Popola Social Footer
        ...
        if (socialContainer) {
            ...
            socialLinks.forEach(link => {
                ...
                a.innerHTML = `<i class="${link.icon || 'fas fa-link'}"></i>`;
        ...
        console.error('Errore caricamento link dinamici:', error);
    } finally {
        // Nascondi sempre lo spinner dopo aver provato a caricare i link
...
(function() {
    ...
    function isStaff() {
        if (window.authSystem && window.authSystem.isStaffLoggedIn()) return true;
        // Fallback ottimistico: identico a auth.js riga 9 — localStorage disponibile prima che window.authSystem sia assegnato
        try { return !!(JSON.parse(localStorage.getItem("clarvs_user"))); } catch(e) { return false; }
    }

    function setCccNav(visible) {
        // Staff: non nascondere MAI, anche se la fetch dice nessuna edizione attiva
        if (!visible && isStaff()) return;
        document.querySelectorAll('#ccc-nav-item').forEach(function(el) {
            el.style.display = visible ? 'block' : 'none';
        });
    }

    function checkCccNav() {
        if (isStaff()) {
            setCccNav(true);
            return;
        }
    ...
    function startPolling() {
        ...
        _pollInterval = setInterval(checkCccNav, 30000);
    }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        var iv = setInterval(function() {
            attempts++;
            if (window.authSystem || attempts > 25) {
                clearInterval(iv);
                checkCccNav();
                startPolling();
            }
    ...
    window.refreshCccNav = checkCccNav;
})();
