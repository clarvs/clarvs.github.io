    ...
    try {
        ...
        container.innerHTML = roster.map(player => {
            ...
            const socials = Object.entries(player.socials || {})
                .filter(([, url]) => url)
                .map(([p, url]) => `<a href="${escHtml(url)}" target="_blank"><i class="${socialIcons[p] || 'fas fa-link'}"></i></a>`)
                .join('');

            const avatar = player.imageUrl
                ? `<div class="player-avatar" style="background-image:url('${escHtml(player.imageUrl)}')"></div>`
            ...
            return `
                <div class="roster-card">
                    <div class="card-content">
                        ${avatar}
                        <h3>${escHtml(player.name)}</h3>
                        <p class="player-role">${escHtml(categoryLabel[player.category] || player.role.toUpperCase())}</p>
                        ${statsHtml}
...
async function loadNews() {
    ...
    newsGrid.innerHTML = newsData.map(news => `
        <article class="news-card">
            <div class="news-image" style="background-image: url('${escHtml(news.image)}')"></div>
            <div class="news-content">
                <h3>${escHtml(news.title)}</h3>
                <time class="news-date">${escHtml(news.date)}</time>
                <p class="news-excerpt">${escHtml(news.content)}</p>
            </div>
        </article>
    `).join('');
}
...
async function loadEvents() {
    ...
    eventsList.innerHTML = eventsData.map(event => `
        <div class="event-card">
            <div class="event-type">${escHtml(event.type)}</div>
            <h3 class="event-title">${escHtml(event.title)}</h3>
            <div class="event-details">
                <div class="event-date"><i class="fas fa-calendar"></i> ${escHtml(event.date)}</div>
                <div class="event-time"><i class="fas fa-clock"></i> ${escHtml(event.time)}</div>
            </div>
        </div>
    `).join('');
}
